...
function SystemPromptsPage() {
  ...
  const fetchPrompts = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Erreur lors du chargement des prompts.');
  ...
  // Filtrage des prompts selon la recherche
  const filteredPrompts = prompts.filter(prompt =>
    (prompt.name && prompt.name.toLowerCase().includes(search.toLowerCase())) ||
...
return (
    <div className="container py-5">
        <div className="d-flex align-items-center justify-content-between mb-4">
            <h1 className="mb-0 text-center flex-grow-1">Prompts Système</h1>
        </div>
        <div className="mb-4" style={{maxWidth: 500, margin: '0 auto'}}>
            <div className="input-group">
                ...
                />
                <button className="btn btn-primary" style={{ minWidth: 100 }} onClick={handleAdd}>Ajouter</button>
            </div>
        </div>
        {showForm && (
            <SystemPromptForm
